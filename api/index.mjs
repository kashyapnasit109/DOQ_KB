...
var dailyReports = sqliteTable("daily_reports", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var equipmentUsage = sqliteTable("equipment_usage", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var materialUsage = sqliteTable("material_usage", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  reportId: integer("report_id").notNull(),
...
var labourRecords = sqliteTable("labour_records", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  reportId: integer("report_id").notNull(),
  category: text("category").notNull(),
  // "centering", "department", "masonry", "plumbing"
...
var paymentRecords = sqliteTable("payment_records", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  reportId: integer("report_id").notNull(),
  category: text("category").notNull(),
  // "masonry_rokdi", "individual", "plumbing"
...
var insertDocumentSchema = createInsertSchema(documents).omit({ id: true });
var insertConversationSchema = createInsertSchema(conversations).omit({ id: true });
var insertSettingSchema = createInsertSchema(settings).omit({ id: true });
var insertUserSchema = createInsertSchema(users).omit({ id: true });
var insertSiteSchema = createInsertSchema(sites).omit({ id: true });
var insertDailyReportSchema = createInsertSchema(dailyReports).omit({ id: true });
var insertEquipmentUsageSchema = createInsertSchema(equipmentUsage).omit({ id: true });
var insertMaterialUsageSchema = createInsertSchema(materialUsage).omit({ id: true });
var insertLabourRecordSchema = createInsertSchema(labourRecords).omit({ id: true });
var insertPaymentRecordSchema = createInsertSchema(paymentRecords).omit({ id: true });
...
var dbUrl = process.env.TURSO_DATABASE_URL ? process.env.TURSO_DATABASE_URL : isVercel ? "file:/tmp/data.db" : "file:data.db";
...
var DatabaseStorage = class {
  ...
  async getDocumentsBySite(siteId) {
    await this.ensureInit();
  ...
  async getDailyReportsByDocument(documentId) {
    await this.ensureInit();
    return await db.select().from(dailyReports).where(eq(dailyReports.documentId, documentId)).all();
  }
  async getDailyReportsBySite(siteId) {
    await this.ensureInit();
    return await db.select().from(dailyReports).where(eq(dailyReports.siteId, siteId)).orderBy(desc(dailyReports.reportDate)).all();
  }
  async getDailyReportsBySiteAndDate(siteId, date) {
    await this.ensureInit();
    return await db.select().from(dailyReports).where(and(eq(dailyReports.siteId, siteId), eq(dailyReports.reportDate, date))).all();
  }
  async getDailyReportsBySiteDateRange(siteId, fromDate, toDate) {
    await this.ensureInit();
    return await db.select().from(dailyReports).where(and(
      eq(dailyReports.siteId, siteId),
      sql`${dailyReports.reportDate} >= ${fromDate}`,
      sql`${dailyReports.reportDate} <= ${toDate}`
    )).orderBy(dailyReports.reportDate).all();
  }
  async deleteDailyReportCascade(reportId) {
    await this.ensureInit();
  ...
  async getPaymentsByReport(reportId) {
    await this.ensureInit();
  ...
  async getSiteSummary(siteId) {
    await this.ensureInit();
    const reportCount = await db.select({ count: sql`count(*)` }).from(dailyReports).where(eq(dailyReports.siteId, siteId)).get();
    const totalCement = await db.select({ total: sql`COALESCE(SUM(${materialUsage.quantityUsed}), 0)` }).from(materialUsage).innerJoin(dailyReports, eq(materialUsage.reportId, dailyReports.id)).where(and(eq(dailyReports.siteId, siteId), like(materialUsage.material, "%cement%"))).get();
    const totalPayments = await db.select({ total: sql`COALESCE(SUM(${paymentRecords.amount}), 0)` }).from(paymentRecords).innerJoin(dailyReports, eq(paymentRecords.reportId, dailyReports.id)).where(eq(dailyReports.siteId, siteId)).get();
    return {
...
import fs from "fs";
var EXTRACTION_PROMPT = `You are an expert at reading handwritten Indian construction site daily reports.
...
async function processAndStoreDocument(docId, filePath, mimeType, siteId, reportDate, apiKey) {
  ...
  const additionalContext = site ? `This report is from site: ${site.name} (${site.code}), date: ${reportDate}` : void 0;
...
async function storeStructuredData(reportId, data) {
...
function buildTextSummary(data, reportDate) {
  let text2 = `Daily Report - ${reportDate}
`;
  text2 += `Reported by: ${data.metadata?.reported_by || "N/A"}

`;
  if (data.equipment_usage?.length) {
    text2 += "EQUIPMENT:\n";
    for (const e of data.equipment_usage) {
      text2 += `  - ${e.equipment}: ${e.working_hours || "N/A"} hours, Diesel: ${e.diesel_used}
`;
    }
    text2 += "\n";
  }
  if (data.material_usage?.length) {
    ...
    for (const m of data.material_usage) {
      text2 += `  - ${m.material}: ${m.quantity_used || "N/A"} ${m.unit || ""} used`;
      if (m.balance != null) text2 += `, balance: ${m.balance}`;
      text2 += "\n";
    }
    text2 += "\n";
  }
  if (data.centering_work?.length) {
    ...
    for (const c of data.centering_work) {
      text2 += `  - ${c.work_type}: ${c.mistri_count} mistri, ${c.helper_count} helper
`;
    }
    text2 += "\n";
  }
  if (data.department_labour?.tasks?.length) {
    text2 += "DEPARTMENT LABOUR:\n";
    for (const t of data.department_labour.tasks) {
      text2 += `  - ${t}
`;
    }
    text2 += `  Total: ${data.department_labour.total_labour} labour, ${data.department_labour.curing_labour} for curing

`;
  }
  if (data.masonry_rokdi) {
    text2 += "MASONRY (ROKDI):\n";
    text2 += `  Total: ${data.masonry_rokdi.total_mistri} mistri, ${data.masonry_rokdi.total_helper} helper
`;
    if (data.masonry_rokdi.total_payment) {
      text2 += `  Payment: \u20B9${data.masonry_rokdi.total_payment} (for ${data.masonry_rokdi.payment_for_dates})
`;
    }
    for (const b of data.masonry_rokdi.breakdown || []) {
      text2 += `    - ${b.location}: ${b.mistri} mistri, ${b.helper} helper
`;
    }
    text2 += "\n";
  }
  if (data.individual_payments?.length) {
    ...
    for (const p of data.individual_payments) {
      text2 += `  - ${p.category}: ${p.count} person(s), \u20B9${p.amount || "N/A"}
`;
    }
    text2 += "\n";
  }
  if (data.summary) {
    text2 += "SUMMARY:\n";
    text2 += `  Cement bags: ${data.summary.total_cement_bags}
`;
    text2 += `  Total mistri: ${data.summary.total_mistri}
`;
    text2 += `  Total helpers: ${data.summary.total_helpers}
`;
    text2 += `  Total labour: ${data.summary.total_labour}
`;
    text2 += `  Total payments: \u20B9${data.summary.total_payments}
`;
    text2 += `  Equipment hours: ${data.summary.equipment_hours}
`;
  }
  return text2;
}
...
async function registerRoutes(httpServer2, app2) {
  ...
  app2.post("/api/auth/update-pin", async (req, res) => {
  ...
  app2.post("/api/admin/settings", async (req, res) => {
    ...
    if (openaiApiKey !== void 0) await storage.setSetting("openai_api_key", openaiApiKey);
    if (adminPin !== void 0) await storage.setSetting("admin_pin", adminPin);
    res.json({ ok: true });
  });
  ...
  app2.get("/api/sites", async (_req, res) => {
    res.json(await storage.getAllSites());
  });
  app2.get("/api/sites/search", async (req, res) => {
    const q = req.query.q || "";
  ...
  app2.get("/api/sites/:id", async (req, res) => {
  ...
  app2.get("/api/sites/:id/reports", async (req, res) => {
    const siteId = Number(req.params.id);
    const { date, from, to } = req.query;
    if (date) return res.json(await storage.getDailyReportsBySiteAndDate(siteId, date));
    if (from && to) return res.json(await storage.getDailyReportsBySiteDateRange(siteId, from, to));
  ...
  app2.post("/api/documents", upload.single("file"), async (req, res) => {
    try {
      ...
      processAndStoreDocument(doc.id, req.file.path, req.file.mimetype, siteId, reportDate, apiKey).catch(async (err) => {
...
Question: ${question}` }
      ...
      const conv = await storage.createConversation({ question, answer, sourceDocs: "[]", createdAt: (/* @__PURE__ */ new Date()).toISOString() });
      res.json(conv);
    } catch (err) {
  ...
  app2.delete("/api/conversations", async (_req, res) => {
